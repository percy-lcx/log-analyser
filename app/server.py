...
def top_urls(
    ...
    where = "" if include_assets else "WHERE url_group NOT IN ('Nuxt Assets','Static Assets')"
    sql = f"""
    ...
    """
    body += f"<p><a href='/export?report=top-urls&from={date_from or ''}&to={date_to or ''}&include_assets={str(include_assets).lower()}&limit={limit}'>Export CSV</a></p>"
...
def top_404(
    ...
    """
    body += f"<p><a href='/export?report=top-404&from={date_from or ''}&to={date_to or ''}&include_assets={str(include_assets).lower()}&limit={limit}'>Export CSV</a></p>"
    body += html_table(rows, cols, max_rows=min(int(limit), 500))
...
def top_5xx(
    ...
    """
    body += f"<p><a href='/export?report=top-5xx&from={date_from or ''}&to={date_to or ''}&include_assets={str(include_assets).lower()}&limit={limit}'>Export CSV</a></p>"
    body += html_table(rows, cols, max_rows=min(int(limit), 500))
...
def wasted_crawl(
    ...
    """
    body += f"<p><a href='/export?report=wasted-crawl&from={date_from or ''}&to={date_to or ''}&strict={str(strict).lower()}&include_assets={str(include_assets).lower()}&bot={bot or ''}&limit={limit}'>Export CSV</a></p>"
    body += html_table(rows, cols, max_rows=min(int(limit), 500))
...
def export(
    ...
    return PlainTextResponse("Unknown report. Try report=daily, top-urls, top-404, top-5xx, bots, wasted-crawl, top-resource-waste", status_code=400)